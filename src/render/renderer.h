...
public:
  ...
  void renderGame(const engine::GameRound& game, engine::Time now);
...
private:
  ...
  math::Mat4x4f getGlobalRotation_(const engine::Player& player, engine::Time now) const;
  float getWaveProgress(const engine::Player & player, engine::Time now) const;
